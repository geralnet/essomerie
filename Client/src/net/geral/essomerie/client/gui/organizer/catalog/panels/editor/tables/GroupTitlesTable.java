package net.geral.essomerie.client.gui.organizer.catalog.panels.editor.tables;

import net.geral.essomerie.client.core.Client;
import net.geral.essomerie.client.core.configuration.CoreConfiguration;
import net.geral.essomerie.client.resources.S;
import net.geral.lib.table.GNTable;

public class GroupTitlesTable extends GNTable<GroupTitlesModel> {
  private static final long serialVersionUID = 1L;

  public GroupTitlesTable() {
    super(new GroupTitlesModel());
  }

  @Override
  protected void createColumns() {
    final CoreConfiguration c = Client.config();
    createColumn(S.ORGANIZER_CATALOG_GROUP_LANGUAGE.s(),
        c.TableColumnWidth_Catalog_GroupTitle_Language,
        c.TableColumnWidth_Default);
    createColumn(S.ORGANIZER_CATALOG_GROUP_TITLE.s(),
        c.TableColumnWidth_Catalog_GroupTitle_Title, c.TableColumnWidth_Default);
    createColumn(S.ORGANIZER_CATALOG_GROUP_SUBTITLE.s(),
        c.TableColumnWidth_Catalog_GroupTitle_Subtitle,
        c.TableColumnWidth_Default);
  }

  @Override
  protected boolean deleteClicked(final int viewRow, final int tableColumn) {
    return true;
  }

  @Override
  public String getNewEntryText(final int columnIndex) {
    switch (columnIndex) {
      case 1:
        return S.ORGANIZER_CATALOG_GROUP_TITLE_DEFAULT.s();
      default:
        return null;
    }
  }
}
