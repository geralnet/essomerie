...
public class BulletinBoardTabPanel extends TabPanel implements ActionListener,
	...
	lblCreatedBy.setBorder(new EtchedBorder(EtchedBorder.LOWERED, null,
		null));
	...
	final int res = JOptionPane.showConfirmDialog(this,
		S.BULLETINBOARD_DELETE, S.TITLE_CONFIRM.s(),
	...
	lblCreatedBy
		.setText(" "
			+ S.BULLETINBOARD_CREATED_BY.s(
				Client.cache().users()
					.get(showing.getCreatedBy()).getName(),
				showing.getCreatedWhen()
					.toString(
						DateTimeFormat
							.forPattern(S.FORMAT_DATETIME_SIMPLE_NO_SECONDS
								.s()))) + " ");
    }
