...
public class RTFEditorPanel extends JPanel implements ActionListener {
	...
	public void actionPerformed(final ActionEvent e) {
		if (!checkAction(e.getActionCommand(), e.getSource())) {
			Log4J.warning(new Exception("Invalid action: " + e.getActionCommand()));
		}
	}
	...
	private boolean applyAttributeSet(final AttributeSet as) {
		...
		rtfPane.getStyleDocument().setCharacterAttributes(offset, length, as, false);
		rtfPane.requestFocus();
		return true;
	}

	private boolean applyColor(final Object source) {
		if (!(source instanceof JButton)) { return false; }
	...
	private boolean checkAction(final String cmd, final Object source) {
		if ("sansserif".equals(cmd)) { return applyFont("sansserif"); }
		if ("monospaced".equals(cmd)) { return applyFont("monospaced"); }
		if ("plain".equals(cmd)) { return applyStyle(false, false); }
		if ("italic".equals(cmd)) { return applyStyle(true, false); }
		if ("bold".equals(cmd)) { return applyStyle(false, true); }
		if ("bolditalic".equals(cmd)) { return applyStyle(true, true); }
		if ("size1".equals(cmd)) { return applySize(12); }
		if ("size2".equals(cmd)) { return applySize(14); }
		if ("size3".equals(cmd)) { return applySize(16); }
		if ("size4".equals(cmd)) { return applySize(20); }
		if ("color".equals(cmd)) { return applyColor(source); }
		return false;
	}
