...
public final class InventoryLogEntry implements Serializable {
	public static String toQuantidadeString(final float qtd) {
		final int i = (int)qtd;
		if (qtd - i == 0f) return String.valueOf(i); //no decimal part
	...
	public InventoryLogEntry(final int _id, final int _iditem, final int _idusuario, final char _tipo,
			final int _idmotivo, final float _quantidadeInicial, final float _variacao, final LocalDateTime _datahora,
	...
	public String getAlteracaoString(final String seta) {
		switch (tipo) {
			case '+':
				return toQuantidadeString(quantidadeInicial) + " + " + toQuantidadeString(variacao) + " = "
						+ toQuantidadeString(quantidadeInicial + variacao);
			case '-':
				return toQuantidadeString(quantidadeInicial) + " - " + toQuantidadeString(variacao) + " = "
						+ toQuantidadeString(Math.max(quantidadeInicial - variacao, 0));
			case '=':
				return toQuantidadeString(quantidadeInicial) + " " + seta + " " + toQuantidadeString(variacao);
			default:
				return "[ERROR " + tipo + "]";
		}
	}
