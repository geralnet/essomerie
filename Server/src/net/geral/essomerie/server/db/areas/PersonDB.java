...
public class PersonDB extends DatabaseArea {
	...
	;
	try (final PreparedResultSet r = db.select(sql, idperson)) {
		...
		final Address a = new Address(id, idperson, postalCode,
			address, suburb, city, state, country, comments,
			classification);
		addresses.add(a);
	    }
	...
	try (PreparedResultSet p = db.select(sql, idperson)) {
	    if (!p.rs.next()) {
		return null;
	    }
	    createdWhen = GNJoda.sqlLocalDateTime(
		    p.rs.getString("updated_when"), false);
	    createdBy = p.rs.getInt("updated_by");
	}
	...
	try (PreparedResultSet p = db.select(sql, idperson)) {
	    ...
	    updatedWhen = GNJoda.sqlLocalDateTime(
		    p.rs.getString("updated_when"), false);
	    updatedBy = p.rs.getInt("updated_by");
	}
	...
	try (final PreparedResultSet r = db.select(sql, idperson)) {
		...
		p = new PersonData(id, type, name, alias, deleted, comments,
			log, telephones, addresses, documents, sales);
	    }
	    if (r.rs.next()) {
	...
	;
	try (final PreparedResultSet r = db.select(sql)) {
	...
	try (final PreparedResultSet p = db.select(sql, t.getNumber())) {
	    final ArrayList<Person> candidates = new ArrayList<>();
	    while (p.rs.next()) {
		candidates.add(get(p.rs.getInt("idperson")));
	    }
	    // TODO ? try reduce candidates by area code
    ...
    public PersonSaleExtended[] getLatestRegistrationExtendedSales(
	    final int entries) throws SQLException {
	final String sql = "SELECT `id`,`idperson`,`price`*100 AS `price`,`when`,`comments`,`updated_by`,`updated_when` " // fields
		+ " FROM `person_sale` " // from
		+ " WHERE `deleted_when` IS NULL " // where
		+ " ORDER BY `updated_when` ASC " // order
		+ " LIMIT ?";
	try (final PreparedResultSet p = db.select(sql, entries)) {
	    final PersonSaleExtended[] sales = new PersonSaleExtended[p
		    .getRowCount()];
	    int i = 0;
	    while (p.rs.next()) {
		...
		final LocalDateTime registered = GNJoda.sqlLocalDateTime(
			p.rs.getString("updated_when"), false);
		sales[i++] = new PersonSaleExtended(iduser, idperson,
			registered, idsale, when, price, comments);
	    }
	    return sales;
	}
    }

    private PersonSaleExtended getSaleExtended(final int idsale)
	    throws SQLException {
	final String sql = "SELECT `idperson`,`price`*100 AS `price`,`when`,`comments`,`updated_by`,`updated_when` " // fields
		+ " FROM `person_sale` " // from
    ...
    public PersonSaleExtended registerSale(final int iduser,
	    final int idperson, final PersonSale sale) throws SQLException {
	final String sql = "INSERT INTO `person_sale` "// insert
		+ "(`idperson`,`price`,`when`,`comments`,`updated_by`,`updated_when`) "// fields
		+ "VALUES (?,?,?,?,?,NOW())";// values
	final int idsale = db.insertLastId(sql, idperson, sale.getPrice(),
		sale.getWhen(), sale.getComments(), iduser);
	return getSaleExtended(idsale);
    }
